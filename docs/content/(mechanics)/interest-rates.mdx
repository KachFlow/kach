...
**Get Rate for Tenor** (`interest_rate.move`):
```move
public fun get_rate_for_tenor(tenor_seconds: u64): u64
```

Returns the rate in basis points for a valid tenor. Aborts if tenor is not supported.

**Validate Tenor** (`interest_rate.move`):
```move
public fun is_valid_tenor(tenor_seconds: u64): bool
```

Checks if a tenor is one of the five supported options.

**Calculate Time-Weighted Interest** (`interest_rate.move`):
```move
...
Calculates interest for a portion of principal over a time period (used in prefund repayments).

**Calculate Early Repayment Discount** (`interest_rate.move`):
```move
...
Calculates the 2% bonus on interest saved through early repayment.

### Constants

```move
// Tenor durations in seconds
...
// Early repayment incentive
const EARLY_REPAYMENT_DISCOUNT_BPS: u64 = 200; // 2%
```

## Related Documentation

- [Prefund PRT System](/mechanics/prefund-prt): Time-weighted interest in action
- [Tranches](/mechanics/tranches): How interest is distributed across risk layers
- [KachFlow Credit Engine](/core/kachflow): Credit system overview
- [Attestator Operations](/mechanics/attestator-operations): Borrowing and repayment workflow

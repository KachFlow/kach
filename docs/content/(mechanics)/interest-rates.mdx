...
description: "Fixed tenor-based interest rates with time-weighted calculations for prefund loans."
---
...
- Time-weighted calculations naturally lower cost when repaid early
...
There is no explicit bonus for early repayment. Time-weighted interest on prefund PRTs already rewards fast repayment by reducing the amount of interest owed and by freeing liquidity for the next draw.
...
2. **Remaining 93%** distributed across tranches using dynamically calculated weighted capital:
   - Multipliers are calculated based on pool composition and base risk premium (governance parameter)
...
**Step 2: Tranche Distribution (dynamically weighted)**
- Example deposits: Senior $80M, Junior $20M
- Base risk premium: 30% (3000 bps)
- Calculated multipliers:
  - Protection ratio: 80M / 20M = 4.0
  - Junior multiplier: 1.0 + (4.0 × 0.30) = 2.2×
  - Inverse ratio: 20M / 80M = 0.25
  - Senior multiplier: 1.0 - (0.25 × 0.30) = 0.925×
- Weights: `w_senior = 80M × 0.925 = 74M`; `w_junior = 20M × 2.2 = 44M`
- Total weight: `118M`
- Senior share: $558 × (74/118) = **$350**
- Junior share: $558 × (44/118) = **$208**

**Per-Dollar Yield** (with weighted capital example above):
- Senior per-dollar: $350 / $80M = 0.0004375% per cycle
- Junior per-dollar: $208 / $20M = 0.00104% per cycle
...
- Average 10-day repayment driven by operational repayment behavior
...
- 7-day average cycle (based on observed repayment speed)
...
2. Faster repayment behavior persists
...
- Impact of repayment speed on velocity and utilization

...
```move
struct PositionNFT<phantom FA> {
    pool_addr: address,           // Which pool this belongs to
    tranche: u8,                  // 0=Senior, 1=Junior
    shares: u64,                  // Share count in this tranche
    nav_index_at_creation: u128,  // [NAV](/glossary#nav) multiplier when created
    lock_until_timestamp: u64,    // Cannot redeem before this time
...
- Lock-up: 90 days
- NFT minted with: tranche=2, shares=9523.81, nav_index=1.05, lock_until=now+7776000
...
object::transfer<PositionNFT<FA>>(
    current_owner,
    position_nft,
    new_owner_address
)
...
**Parameters**:
- `position_addr`: Address of the Position NFT to split
- `shares_for_new`: How many shares go to the new NFT
...
public entry fun merge_positions<FA>(
    owner: &signer,
    position1_addr: address,
    position2_addr: address
)
```
...
```move
public entry fun redeem_position<FA>(
    owner: &signer,
    position_addr: address
)
```
...
```move
public fun get_position_details<FA>(
    position_addr: address
): (address, u8, u64, u128, u64, u64)
```
...
### Check if Redeemable
```move
public fun is_redeemable<FA>(position_addr: address): bool
```
...
**Events Emitted**:
```move
struct PositionMinted<phantom FA> {
    position_addr: address,
    owner: address,
    pool: address,
    tranche: u8,
...
struct PositionSplit<phantom FA> {
...
struct PositionsMerged<phantom FA> {
    kept_addr: address,
    burned_addr: address,
    total_shares: u64,
    merged_nav: u128
...
struct PositionRedeemed<phantom FA> {
    position_addr: address,
    owner: address,
    shares: u64,
    amount_redeemed: u64
...
- [Tranches & Risk Layers](/mechanics/tranches): Understanding risk/return profiles
- [Pool Mechanics](/mechanics/architecture): How pools manage liquidity
- [Lifecycle](/core/lifecycle): Full user journey including deposits

...
1.  **Deposit**
    ...
    -   A depositor selects the stablecoin they want to allocate (USDC at launch, more via governance) along with a [tranche](/glossary#tranche) (senior or junior) and a lock-up (flexible, 3, 6, 12, 24 months).

    -   The depositor sends the stablecoin to the pool. The [contract](https://github.com/KachFlow/kach) mints a [Position NFT](/glossary#position-nft) with the stablecoin id, [tranche](/glossary#tranche), shares, lock-up, [NAV](/glossary#nav) index, and timestamps.
...
4.  **Repayment**
    ...
    -   Within the selected tenor (standard 1–5 days; prefund/extended options fixed at 7/14/30/60/90 days), the Attestator repays principal plus interest in the same stablecoin. The PRT is burned, and repayment events record realized yield or penalties.
...
5.  **Redistribution**
    ...
    -   [NAV](/glossary#nav) and yield distribution reference NFT metadata so each position accrues exactly what its [tranche](/glossary#tranche), lock-up, and [NAV](/glossary#nav) index dictate. Repaid principal becomes available for the next credit cycle, subject to [tranche](/glossary#tranche) and lock-up constraints.
...
-   Periodic attestations (auditor-reviewed when feasible) reconcile [NAV](/glossary#nav), cash balances, and outstanding PRTs, ensuring that passive LPs are never "flying blind" even though underwriting decisions are centralized.

...
Every Attestator (borrower) has a **Trust Score** that governs credit access and draw capabilities. The full hybrid model (amount- and count-weighted scoring, severity multipliers, recency decay, confidence scaling, anti-gaming sizing, and concave per-loan weighting) is documented in [Mechanics → Trust Score](/mechanics/trust-score).

module kach::trust_score {
    ...
    public fun update_trust_score(
        ...
        let effective_amount = calculate_effective_amount(
            loan_amount, governance_address
        );
    ...
    fun apply_decay(
        trust_score: &mut TrustScore, governance_address: address
    ) {
        ...
        let decay_interval_seconds =
            governance::get_trust_decay_interval_seconds(governance_address);
        ...
        if (intervals_passed > 0) {
            ...
            while (i < intervals_passed && i < 100) { // Cap at 100 intervals for safety
                trust_score.good_volume =
                    (trust_score.good_volume * (decay_factor_bps as u128)) / 10000;
                trust_score.late_volume =
    ...
    fun calculate_effective_amount(
        loan_amount: u64, governance_address: address
    ): u128 {
    ...
    fun calculate_trust_score_internal(
        trust_score: &TrustScore, governance_address: address
    ): u64 {
        ...
        let volume_weight_bps =
            governance::get_trust_volume_weight_bps(governance_address);
        ...
        let volume_score_raw =
            if (base_score_scaled > volume_penalty) {
        ...
        let confidence_divisor =
            governance::get_trust_confidence_divisor(governance_address);
        let confidence_bonus_bps =
        ...
        let confidence_bps =
            if (confidence_bonus_bps > 20000) { 20000 }
            else {
    ...
    public fun get_max_loan_amount(
        ...
        let earned_capacity = (trust_score.good_volume * (k_bps as u128)) / (
            10000 * SCALE
        );
    ...
    public fun get_trust_score_details(
        ...
        let max_loan =
            if (trust_score.total_loans < BOOTSTRAP_LOAN_COUNT) {
                ...
                let k_bps = governance::get_trust_anti_gaming_k_bps(governance_address);
                let earned_capacity =
                    (trust_score.good_volume * (k_bps as u128)) / (10000 * SCALE);
                let earned_capacity_u64 = (earned_capacity as u64);
                if (trust_score.approved_credit_limit < earned_capacity_u64) {
                    trust_score.approved_credit_limit
                } else {
